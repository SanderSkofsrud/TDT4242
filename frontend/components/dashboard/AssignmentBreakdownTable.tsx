interface AssignmentBreakdownRow {
  assignmentId: string
  title?: string
...
interface AssignmentBreakdownTableProps {
  rows: AssignmentBreakdownRow[]
}

export function AssignmentBreakdownTable({ rows }: AssignmentBreakdownTableProps) {
  if (rows.length === 0) {
    return <p className="text-slate-600">No assignment-level usage data yet.</p>
  }
        ...
        <tbody>
          {rows.map((row) => {
            ...
            const frequencyText = Object.entries(row.byFrequency)
              ...
              .join(', ')
            return (
              <tr key={row.assignmentId} className="border-b border-slate-100">
                <td className="py-3 px-3 text-slate-700">
                  {row.title ?? row.assignmentId}
                </td>
